...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    const MATCH: &str = "Couldn't find this account";
    const INVALID_CHARS: [char; 31] = [
        '!', '"', '#', '$', '%', '&', '\'', '(', ')', '*', '+', ',', '-', '.', '/', ':', ';', '<',
        '=', '>', '?', '@', '[', '\\', ']', '^', '`', '{', '|', '}', '~',
    ];
    ...
    for user in users.lines() {
        ...
        let resp = reqwest::get(format!("https://tiktok.com/@{}", user))
            .await?
            .text()
            .await?;
        if resp.type_id() == TypeId::of::<reqwest::Error>() {
            println!("[ + ] -> tiktok.com/@{} -- CONNECTION ERROR", user);
            continue;
        }
        if resp.contains(MATCH) {
            println!("[ + ] -> tiktok.com/@{} -- FREE / BANNED", user);
